	...
	myjqGrid: function(opt, callback) {
		...
		var defaults = {
			...
			gridComplete: function() { //当表格所有数据都加载完成，处理统计行数据
				...
				$('.ui-icon-seek-prev').attr('style', 'background-image:url("/static/pub/jqGrid/image/' + prev_img + '.png")');
				$('.ui-icon-seek-next').attr('style', 'background-image:url("/static/pub/jqGrid/image/' + next_img + '.png")');
